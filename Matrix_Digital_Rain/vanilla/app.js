...
CHARACTERS =
  "日アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲンあいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやゆよらりるれろわをん";
...
function Drop() {
  ...
  this.dropLength =
    Math.floor(
      Math.random() *
        ((canvas.height / charSize) * 3 - canvas.height / charSize + 1)
    ) +
...
function rainAnimate() {
  ...
  numDrops =
    Math.floor(Math.random() * (maxNumDrops - minNumDrops + 1)) + minNumDrops;
  drops = [];
  ...
  changeInterval = setInterval(function () {
    ...
    for (var d = 0; d < drops.length; d++) {
      ...
      randomChar[d] = CHARACTERS.charAt(
...
function drawChars(drops) {
  ...
  for (var n = 0; n < drops.length; n++) {
    ...
    if (drop.dropChars.length == 0) {
      ...
      drop.dropChars[previousChar.index + 1] = new Char(
        drop.dropXPos,
        previousChar.yPos + charSize,
        previousChar.index + 1
      );
    }
    ...
    if (drop.dropChars.length > 1) {
      ...
      for (
        currentChar = drop.dropChars.length - 1, currentFillStyle = 0;
        currentChar >= 0 && currentFillStyle < 5;
        currentFillStyle++, currentChar--
      ) {
    ...
    if (drop.dropChars.length > drop.dropLength) {
      ...
      for (
        var f = 0;
        f < drop.numToFade && drop.fadeIndex + f < drop.dropChars.length;
        f++
      ) {
    ...
    for (var c = 0; c < drop.dropChars.length; c++) {
      ...
      ctx.fillText(
        drop.dropChars[c].char,
        drop.dropChars[c].xPos,
        drop.dropChars[c].yPos
      );
    }
